...
| 구분 | 항목 | 설명 | 기본값 |
|------|------|------|--------|
| 선택 | 히스토리 파일 경로 | Claude 세션 히스토리 `.jsonl` 파일이 위치한 디렉토리 | `~/.claude/projects/` 하위 |
| 선택 | 학습 로그 파일 경로 | 주간 학습 로그 마크다운 파일 경로 | `{설정의 학습 로그 경로}` |
...
```bash
# 오늘 수정된 세션 파일 찾기
find {설정의 히스토리 경로} -name "*.jsonl" -maxdepth 2 -mtime 0

# 이미 기록된 세션 ID 확인 (중복 방지)
grep "session-id:" {설정의 학습 로그 경로} 2>/dev/null
```
