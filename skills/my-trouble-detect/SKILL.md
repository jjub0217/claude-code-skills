...
`{설정의 트러블 리포트 경로}/YYYY-MM-DD-에러-키워드.md`에 저장됩니다.
...
- **과거 트러블 비교**: `{설정의 트러블 리포트 경로}` 폴더가 있으면 과거 기록과 비교하여 반복 패턴 감지
- **출력 형식**:
  ```
  🔍 분석 결과
  - 추정 원인: (1순위 원인)
...
- **리포트 저장**: `{설정의 트러블 리포트 경로}/YYYY-MM-DD-제목.md`로 저장 (나중에 패턴 분석에 활용)
- **건너뛰기**: 리포트만 확인하고 종료
...
3. "리포트 저장"을 선택하면, `{설정의 트러블 리포트 경로}` 폴더에 마크다운 파일로 저장한다
